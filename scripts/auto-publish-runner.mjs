...
- 카테고리: ${category}
- 작성 지침 (★ AI 특유 문체 및 서식 엄격 금지):
  1. '${category}' 분야에서 국민들이 가장 많이 검색하고 실질적 혜택(환급금, 지원금, 공과금 절약, 고금리 등)을 즉시 받을 수 있는 실전 주제를 선정하세요.
  2. [필수 포함] 지원 대상 자격 요건 표(Table), 구체적 지원 금액 수치, 모바일/온라인 3단계 신청 경로, 중도해지나 신청 누락 주의사항.
  3. [절대 금지] 기계적인 '들어가며', '마치며', '서론', '결론' 헤딩을 절대 쓰지 마세요. 상투적인 인트로/클로징 멘트('~에 대해 알아보겠습니다', '이 글에서는 ~를 정리합니다', '~해 보시기 바랍니다', '도움이 되셨기를 바랍니다')도 전면 금지합니다.
  4. [절대 금지] 불필요한 공백/자간(단어 앞뒤 두 칸 이상 공백)을 넣지 마세요.
  5. [절대 금지] 문장마다 키워드에 볼드체(**단어**)를 남발하지 마세요. 메뉴 경로나 액수는 인라인 코드(\`code\`)로 표기하고, 볼드는 본문 전체에서 가장 중요한 결론 1~2개에만 극도로 절제하세요.
  6. 소제목에 '1.', '1.1', '2.' 식의 관료적 번호 매기기를 하지 말고 직관적인 텍스트 소제목을 쓰세요.
  7. 완성된 글은 '/workspace/blogs/content/posts/[고유-영문-슬러그].md' 파일로 저장하세요.
  8. 글 작성이 완료되면 파일 경로와 제목, 슬러그를 명시하며 완료를 알리세요.
`.trim();
